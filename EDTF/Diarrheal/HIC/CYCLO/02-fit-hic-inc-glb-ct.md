Diarrheal Incidence Cyclospora • Model fit
================
CaDi2575
2025-05-08
...
```
    ...
    ## TMPDIR=C:/Users/CaDi2575/AppData/Local/Temp/RtmpEtlv7o/file362c7b1a7da7 -V' had status 1
...
saveRDS(es, paste0(params$Dir, sprintf("/es_hic_inc_%s.rds", bd::today())))
```
...
```
    ...
    ##  -2.4407    4.8815    8.8815    6.2678   20.8815   
    ## 
    ## Variance Components:
    ## 
    ##             estim    sqrt  nlvls  fixed  factor 
    ## sigma^2    0.6721  0.8198      3     no      ID 
    ## 
    ## Test for Heterogeneity:
    ## Q(df = 2) = 13794.4015, p-val < .0001
    ## 
    ## Model Results:
    ## 
    ## estimate      se    tval  df    pval   ci.lb   ci.ub    
    ##   2.3146  0.4734  4.8897   2  0.0394  0.2779  4.3513  * 
    ## 
    ## ---
...
```
    ...
    ## 
    ##   logLik  Deviance       AIC       BIC      AICc   
    ##  -1.3816    2.7632    8.7632    2.7632   32.7632   
    ## 
    ## Variance Components:
    ## 
    ##             estim    sqrt  nlvls  fixed  factor 
    ## sigma^2    0.9280  0.9633      3     no      ID 
    ## 
    ## Test for Residual Heterogeneity:
    ## QE(df = 1) = 11025.7881, p-val < .0001
    ## 
    ## Test of Moderators (coefficient 2):
    ## F(df1 = 1, df2 = 1) = 0.4487, p-val = 0.6243
    ## 
    ## Model Results:
    ## 
    ##           estimate        se     tval  df    pval       ci.lb      ci.ub    
    ## intrcpt  -173.5844  262.5841  -0.6611   1  0.6281  -3510.0314  3162.8627    
    ## YEAR        0.0878    0.1311   0.6699   1  0.6243     -1.5779     1.7535    
    ## 
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
...
```
    ...
    ## Full     3 8.7632 2.7632 32.7632 -1.3816               11025.7881 
    ## Reduced  2 8.8815 6.2678 20.8815 -2.4407 2.1183 0.1456 13794.4015
...
![](02-fit-hic-inc-glb-ct_files/figure-gfm/r_lineartimetrend.png)
...
```
    ...
    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : span too small.  fewer data values
    ## than degrees of freedom.

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : pseudoinverse used at 1997

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : neighborhood radius 9.045

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : reciprocal condition number 0

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : at 2006

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : radius 0.002025

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : all data on boundary of neighborhood.
    ## make span bigger

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : There are other near singularities as
    ## well. 0.002025

    ## Warning in simpleLoess(y, x, w, span, degree = degree, parametric = parametric, : zero-width neighborhood. make span
    ## bigger
...
![](02-fit-hic-inc-glb-ct_files/figure-gfm/r_nonlineartimetrend.png)
...
if (params$Level == "REG_YR") {
  fit_brms_reg_s <-
    brm(yi | se(sei) ~ 1 + YEAR + (1 | REG2) + (1 | REG2:ID),
        ...
        open_progress = FALSE,
        seed = 7)
  saveRDS(fit_brms_reg_s, file = paste0(params$Dir, "/fit_brms_inc_reg_yr.rds"))
  ...
  fit_brms_reg_s <-
    brm(yi | se(sei) ~ 1 + YEAR + (1 | ID),
        ...
        open_progress = FALSE,
        seed = 7)
  saveRDS(fit_brms_reg_s, file = paste0(params$Dir, "/fit_brms_inc_glb_yr.rds"))
  ...
  fit_brms_reg_s <-
    brm(yi | se(sei) ~ 1 + (1 | REG2) + (1 | REG2:ID),
        ...
        open_progress = FALSE,
        seed = 7)
  saveRDS(fit_brms_reg_s, file = paste0(params$Dir, "/fit_brms_inc_reg_ct.rds"))
  ...
  fit_brms_reg_s <-
    brm(yi | se(sei) ~ 1 + (1 | ID),
        ...
        open_progress = FALSE,
        seed = 7)
  saveRDS(fit_brms_reg_s, file = paste0(params$Dir, "/fit_brms_inc_glb_ct.rds"))
}
```
...
```
    ...
    ## Formula: yi | se(sei) ~ 1 + (1 | ID) 
    ##    Data: subset(es, as.integer(FLAG) == 1) (Number of observations: 3) 
    ...
    ## sd(Intercept)     0.99      0.42     0.43     2.05 1.00     1371     1929
    ## 
    ## Regression Coefficients:
    ##           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## Intercept     2.27      0.59     1.03     3.46 1.00     1214     1197
    ## 
...
![](02-fit-hic-inc-glb-ct_files/figure-gfm/r_summary-1.png)![](02-fit-hic-inc-glb-ct_files/figure-gfm/r_summary-2.png)![](02-fit-hic-inc-glb-ct_files/figure-gfm/r_summary-3.png)
...
```
    ...
    ## Warning in system2("quarto", "-V", stdout = TRUE, env = paste0("TMPDIR=", : running command '"quarto"
    ## TMPDIR=C:/Users/CaDi2575/AppData/Local/Temp/RtmpEtlv7o/file362c5a1cf35 -V' had status 1
    ...
    ##  quarto   ERROR: Unknown command "TMPDIR=C:/Users/CaDi2575/AppData/Local/Temp/RtmpEtlv7o/file362c5a1cf35". Did you mean command "preview"? @ C:\\PROGRA~1\\RStudio\\RESOUR~1\\app\\bin\\quarto\\bin\\quarto.exe
    ## 
    ## ─ Packages ────────────────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    ##    bd             * 0.0.14     2025-05-08 [1] Github (brechtdv/bd@652191c)
    ...
    ##    FERG2          * 0.0.4      2025-05-08 [1] Github (brechtdv/FERG2@be891dc)
    ...
    ## 
    ##  [1] C:/Users/CaDi2575/AppData/Local/Programs/R/R-4.5.0/library
    ## 
