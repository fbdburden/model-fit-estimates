Global incidence of Campylobacter (Diarrheal) • Estimates
================
fbbu6966
2025-10-11
...
fit_brms_reg_s <- readRDS(file.path(params$Dir, "fit_brms_inc_reg_yr.rds"))
...
```
    ...
    ## Formula: yi | se(sei) ~ 1 + YEAR + (1 | REG2) + (1 | REG2:ID) 
    ##    Data: subset(es, as.integer(FLAG) == 1) (Number of observations: 34) 
    ...
    ## sd(Intercept)     0.39      0.36     0.01     1.33 1.01      310      378
    ## 
    ## ~REG2:ID (Number of levels: 27) 
    ##               Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## sd(Intercept)     0.68      0.10     0.51     0.90 1.01     1338     2427
    ## 
    ## Regression Coefficients:
    ##           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
    ## Intercept    -0.46      5.59   -11.34    10.59 1.00     7274     6153
    ## YEAR          0.00      0.00    -0.00     0.01 1.00     7314     6221
    ## 
...
``` r
zero_cases_reg2 <-
table(zero_cases$REG2, zero_cases$DISEASEFREE, useNA = "always") %>%
  as.data.frame() %>%
  mutate(REG2 = Var1, 
         Var2 = case_when(
           Var2 == 0 ~ "Nonendemic",
...
zero_cases_reg2 <-
left_join(zero_cases_reg2, Reg2_with_data) %>%
...
saveRDS(sim_all, sprintf("%s/sim_all_inc_reg_yr_%s.rds", params$Dir, bd::today()))
...
```
    ...
    ##  $ VAL_MEAN     : num  8779159 8867563 8958994 9053702 9148412 ...
    ##  $ VAL_LWR      : num  6754240 6837765 6916148 6998565 7072421 ...
    ##  $ VAL_UPR      : num  11396011 11507922 11630071 11743338 11845377 ...
...
```
    ...
    ## 'data.frame':    22 obs. of  7 variables:
    ##  $ YEAR         : int  2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 ...
    ##  $ VAL_MEAN     : num [1:22(1d)] 831 834 837 840 843 ...
    ##  $ VAL_LWR      : num [1:22(1d)] 639 643 646 649 651 ...
    ##  $ VAL_UPR      : num [1:22(1d)] 1078 1082 1086 1089 1091 ...
    ##  $ LOCATION     : chr  "Global" "Global" "Global" "Global" ...
    ##  $ LOCATION_NAME: chr  "Global" "Global" "Global" "Global" ...
...
```
    ...
    ##  $ VAL_MEAN     : num  0 2700943 219585 3837407 0 ...
    ##  $ VAL_LWR      : num  0 1688601 111176 2651796 0 ...
    ##  $ VAL_UPR      : num  0 4044738 403619 5287938 0 ...
...
```
    ...
    ## 'data.frame':    132 obs. of  7 variables:
    ##  $ YEAR         : int  2000 2000 2000 2000 2000 2000 2001 2001 2001 2001 ...
    ##  $ VAL_MEAN     : num  NaN 806 892 773 NaN ...
    ##  $ VAL_LWR      : num  NaN 504 452 534 NaN ...
    ##  $ VAL_UPR      : num  NaN 1206 1640 1066 NaN ...
    ##  $ LOCATION     : chr  "Region" "Region" "Region" "Region" ...
    ##  $ LOCATION_NAME: chr  "AFR" "AMR" "EMR" "EUR" ...
    ##  $ METRIC       : chr  "Rate" "Rate" "Rate" "Rate" ...
...
```
    ...
    ##  $ YEAR         : int  2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 ...
    ##  $ VAL_MEAN     : num  831 834 837 840 843 ...
    ##  $ VAL_LWR      : num  639 643 646 649 651 ...
    ##  $ VAL_UPR      : num  1078 1082 1086 1089 1091 ...
    ##  $ LOCATION     : chr  "Global" "Global" "Global" "Global" ...
    ##  $ LOCATION_NAME: chr  "Global" "Global" "Global" "Global" ...
    ##  $ METRIC       : chr  "Rate" "Rate" "Rate" "Rate" ...
...
|------:|-----------:|----------:|-----------:|
| 2,010 |  9,772,975 | 7,577,098 | 12,680,960 |
| 2,020 | 10,785,703 | 8,275,138 | 14,075,684 |

Global number of Campylobacter cases, 2010 vs 2020
...
Incidence per 100,000 of Campylobacter by WHO region in 2020
</caption>
...
Cases of Campylobacter by WHO region in 2020
</caption>
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CI_2010.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CI_2010.png)
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CI_2020.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CI_2020.png)
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CASES_2010.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CASES_2010.png)
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CASES_2020.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_CASES_2020.png)
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_hist_2010.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_hist_2010.png)
...
``` r
setwd(params$Dir)
image <- paste0("03-estimate-hic-inc-reg-yr_files/figure-gfm/r_hist_2020_2010.png")
cat("![](",image,")")
```

![](03-estimate-hic-inc-reg-yr_files/figure-gfm/r_hist_2020_2010.png)
...
```
    ...
    ## TMPDIR=C:/Users/fbbu6966/AppData/Local/Temp/RtmpGq2HuE/file43d045ba6f44 -V' had status 1
    ...
    ##  quarto   ERROR: Unknown command "TMPDIR=C:/Users/fbbu6966/AppData/Local/Temp/RtmpGq2HuE/file43d045ba6f44". Did you mean command "update"? @ C:\\Users\\fbbu6966\\AppData\\Local\\Programs\\RStudio\\RESOUR~1\\app\\bin\\quarto\\bin\\quarto.exe
    ## 
    ## ─ Packages ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
