...
#   dalymod:::get_samples(
#     n_samples,
#     "../../ESTIMATES_HIC/04-eaec/sim_all_inc_glb_ct_20250508.rds",
#     transformation = "log",
#     denominator = 1e5)
# summarize(hic_inc)
# 
...
for (year in as.character(all_yrs)) {
  ...
  for (country in countries_lmic) {
    ...
    # MRT_NR
    # dalycalc[[country]][[1]]$MRT_NR <- 0
    # dalycalc[[country]][[2]]$MRT_NR <- 0
    dalycalc[[country]][[1]]$MRT_NR <-
      dalycalc[[country]][[1]]$MRT_NR *
      subset(lmic_af_ip,
             AGE == "ch" & COUNTRY == country & YEAR == year)$SAMPLES[[1]]
...
##bd::render_today("edtf-diarrhea-eaec-daly.R")