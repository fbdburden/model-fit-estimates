### FERG2/EDTF/DIARRHEA/ROTAVIRUS/DALY
...
hic_inc <-
  dalymod:::get_samples(
    n_samples,
    "../../ESTIMATES_HIC/10-rota/sim_all_inc_reg_ct_20250518.rds",
    transformation = "log",
    denominator = 1e5)
summarize(hic_inc)
...
hic_mrt <-
  dalymod:::get_samples(
    n_samples,
    "../../ESTIMATES_HIC/10-rota/sim_all_mrt_glb_ct_20250518.rds",
    transformation = "log",
    denominator = 1e5)
summarize(hic_mrt)
...
hic_age <-
  list(`2000` = c("<5" = 0.597, "5-9" = 0.101, "10-19" = 0.030, "20-64" = 0.153, "65+" = 0.119),
       `2010` = c("<5" = 0.597, "5-9" = 0.101, "10-19" = 0.030, "20-64" = 0.153, "65+" = 0.119),
       `2021` = c("<5" = 0.633, "5-9" = 0.054, "10-19" = 0.031, "20-64" = 0.183, "65+" = 0.098))
hic_age <-
...
## get source attribution estimates / (set all to EMRA)
sa <- get_sa(hazard_sa, n_samples)
# sa <- lapply(sa, function(x) sa$EMRA)
# knitr::kable(t(sapply(sa, mean_ci)))
# sa_file <-
# "\\\\sciensano.be/fs/11401_LifeChron_FERG_DWH/SA/20250517/ROTAV/ROTAV_GLOBAL_SAMPLES.rds"
# sa_glb <- readRDS(sa_file)[, "Food"] / 100
# sa <- lapply(sa, function(x) sa_glb)
knitr::kable(t(sapply(sa, mean_ci)))
...
##bd::render_today("edtf-diarrhea-rota-daly.R")