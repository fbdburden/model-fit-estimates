### FERG2/EDTF/DIARRHEA/NOROVIRUS/DALY
...
hic_inc <-
  dalymod:::get_samples(
    n_samples,
    "../../ESTIMATES_HIC/09-noro/sim_all_inc_glb_yr_20250518.rds",
    transformation = "log",
    denominator = 1e5)
summarize(hic_inc)
...
hic_mrt <-
  dalymod:::get_samples(
    n_samples,
    "../../ESTIMATES_HIC/09-noro/sim_all_mrt_reg_ct_20251208.rds",
    transformation = "log",
    denominator = 1e5)
summarize(hic_mrt)
...
# define age distribution - no time trend
hic_age <-
  list(`2000` = c("<5" = 0.167, "5-14" = 0.117, "15-64" = 0.527, "65+" = 0.188),
       `2010` = c("<5" = 0.167, "5-14" = 0.117, "15-64" = 0.527, "65+" = 0.188),
       `2021` = c("<5" = 0.167, "5-14" = 0.117, "15-64" = 0.527, "65+" = 0.188))
hic_age <-
...
rle_ad_hic <-
  rbind(cbind(AGE = "5-14", get_weighted_rle("5-14")),
        cbind(AGE = "15-64", get_weighted_rle("15-64")),
...
## YLD settings / vomiting
p_vomit <- dalymod:::sim_pert(n_samples, 0.19, 0.14, 0.23)
mean_ci(p_vomit)
dsw_vomit <- dalymod:::sim_mean_ci(n_samples, 0.074, 0.049, 0.104, "PROB")
mean_ci(dsw_vomit)
...
for (year in as.character(all_yrs)) {
  ...
  for (country in names(dalycalc)) {
    ...
    dalycalc[[country]][[1]]$YLD_NR <-
      dalycalc[[country]][[1]]$YLD_NR +
        dalycalc[[country]][[1]]$INC_NR * p_vomit * dur_ch * dsw_vomit
    dalycalc[[country]][[2]]$YLD_NR <-
      dalycalc[[country]][[2]]$YLD_NR +
        dalycalc[[country]][[2]]$INC_NR * p_vomit * dur_ad * dsw_vomit
    
    # INC_NR
    dalycalc[[country]][[1]]$INC_NR <-
      dalycalc[[country]][[1]]$INC_NR +
        dalycalc[[country]][[1]]$INC_NR * p_vomit
    dalycalc[[country]][[2]]$INC_NR <-
      dalycalc[[country]][[2]]$INC_NR +
        dalycalc[[country]][[2]]$INC_NR * p_vomit
...
##bd::render_today("edtf-diarrhea-noro-daly.R")