Methylmercury Profound • Models fit
================
LoVa3397
2025-09-15
...
```
    ...
    ## Warning in system2("quarto", "-V", stdout = TRUE, env = paste0("TMPDIR=", : running command '"quarto" TMPDIR=C:/Users/LoVa3397/AppData/Local/Temp/RtmpUrHJ3m/file20a810167d6e
    ## -V' had status 1
...
saveRDS(es, paste0(params$Dir,"/es_v9.RDS"))
...
``` r
setwd(params$Dir)
image <- paste0("02-fit_v9_files/figure-gfm/r_lineartimetrend.png")
cat("![](",image,")")
```

![](02-fit_v9_files/figure-gfm/r_lineartimetrend.png)
...
``` r
setwd(params$Dir)
image <- paste0("02-fit_v9_files/figure-gfm/r_nonlineartimetrend.png")
cat("![](",image,")")
```

![](02-fit_v9_files/figure-gfm/r_nonlineartimetrend.png)
...
``` r
saveRDS(fit_brms_reg_s, file = paste0(params$Dir, "/fit_brms_country_s_v9.rds"))
...
setwd(params$Dir)
for (i in 1:length){
  image <- paste0("02-fit_v9_files/figure-gfm/r_summary-",i,".png")
  cat("![](",image,")")
}
```

![](02-fit_v9_files/figure-gfm/r_summary-1.png)![](02-fit_v9_files/figure-gfm/r_summary-2.png)![](02-fit_v9_files/figure-gfm/r_summary-3.png)![](02-fit_v9_files/figure-gfm/r_summary-4.png)![](02-fit_v9_files/figure-gfm/r_summary-5.png)![](02-fit_v9_files/figure-gfm/r_summary-6.png)![](02-fit_v9_files/figure-gfm/r_summary-7.png)
...
if (params$Level != "NOY"){
  for (i in 1:length){
    image <- paste0("02-fit_v9_files/figure-gfm/r_conditional_effects-",i,".png")
    cat("![](",image,")")
  }
}
...
vector\<lower=0\>\[N\] se; // known sampling error // data for
group-level effects of ID 1 int\<lower=1\> N_1; // number of grouping
levels int\<lower=1\> M_1; // number of coefficients per level
array\[N\] int\<lower=1\> J_1; // grouping indicator per observation //
group-level predictor values vector\[N\] Z_1_1; // data for group-level
effects of ID 2 int\<lower=1\> N_2; // number of grouping levels
int\<lower=1\> M_2; // number of coefficients per level array\[N\]
int\<lower=1\> J_2; // grouping indicator per observation // group-level
predictor values vector\[N\] Z_2_1; // data for group-level effects of
ID 3 int\<lower=1\> N_3; // number of grouping levels int\<lower=1\>
M_3; // number of coefficients per level array\[N\] int\<lower=1\> J_3;
// grouping indicator per observation // group-level predictor values
vector\[N\] Z_3_1; // data for group-level effects of ID 4
int\<lower=1\> N_4; // number of grouping levels int\<lower=1\> M_4; //
number of coefficients per level array\[N\] int\<lower=1\> J_4; //
grouping indicator per observation // group-level predictor values
vector\[N\] Z_4_1; // data for group-level effects of ID 5
int\<lower=1\> N_5; // number of grouping levels int\<lower=1\> M_5; //
number of coefficients per level array\[N\] int\<lower=1\> J_5; //
grouping indicator per observation // group-level predictor values
vector\[N\] Z_5_1; int prior_only; // should the likelihood be ignored?
} transformed data { vector\<lower=0\>\[N\] se2 = square(se); }
parameters { real Intercept; // temporary intercept for centered
...
(z_5\[1\])); lprior += student_t_lpdf(Intercept \| 3, -11.4, 3.4);
lprior += normal_lpdf(sd_1 \| 0, 1) - 1 \* normal_lccdf(0 \| 0, 1);
lprior += normal_lpdf(sd_2 \| 0, 1) - 1 \* normal_lccdf(0 \| 0, 1);
lprior += normal_lpdf(sd_3 \| 0, 1) - 1 \* normal_lccdf(0 \| 0, 1);
lprior += normal_lpdf(sd_4 \| 0, 1) - 1 \* normal_lccdf(0 \| 0, 1);
lprior += normal_lpdf(sd_5 \| 0, 1) - 1 \* normal_lccdf(0 \| 0, 1); }
model { // likelihood including constants if (!prior_only) { //
initialize linear predictor term vector\[N\] mu = rep_vector(0.0, N); mu
+= Intercept; for (n in 1:N) { // add more terms to the linear predictor
mu\[n\] += r_1_1\[J_1\[n\]\] \* Z_1_1\[n\] + r_2_1\[J_2\[n\]\] \*
Z_2_1\[n\] + r_3_1\[J_3\[n\]\] \* Z_3_1\[n\] + r_4_1\[J_4\[n\]\] \*
Z_4_1\[n\] + r_5_1\[J_5\[n\]\] \* Z_5_1\[n\]; } target += normal_lpdf(Y
\| mu, se); } // priors including constants target += lprior; target +=
...
```
    ...
    ## Warning in system2("quarto", "-V", stdout = TRUE, env = paste0("TMPDIR=", : running command '"quarto" TMPDIR=C:/Users/LoVa3397/AppData/Local/Temp/RtmpUrHJ3m/file20a87e2a3eda
    ## -V' had status 1
    ...
    ##  quarto   ERROR: Unknown command "TMPDIR=C:/Users/LoVa3397/AppData/Local/Temp/RtmpUrHJ3m/file20a87e2a3eda". Did you mean command "create"? @ C:\\PROGRA~1\\RStudio\\RESOUR~1\\app\\bin\\quarto\\bin\\quarto.exe
    ## 
    ## ─ Packages ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
